...
export default function NewLoadPage() {
  ...
  const fetchProviders = async () => {
    try {
      const response = await fetch('/api/providers?page=1&limit=1000') // Get all providers for the dropdown
      if (response.ok) {
  ...
  const fetchTrucks = async () => {
    try {
      const response = await fetch('/api/trucks?page=1&limit=1000') // Get all trucks for the dropdown
      if (response.ok) {
        const data = await response.json()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setError(data.error || 'Error al registrar carga')
      }
    } catch (error) {
  ...
  return (
    <AppLayout>
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-900">Nueva Carga</h1>
          <p className="mt-2 text-sm text-gray-700">
            Registra una nueva carga de camión.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Cantidad de la carga"
                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm h-10 px-3 text-gray-900"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contenedora de la carga"
                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm h-10 px-3 text-gray-900"
              />
            </div>
