  ...
  const fetchEntry = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setEntry(entryData)
        setSelectedProvider(entryData.providerId)
        setSelectedTruck(entryData.truck.licensePlate)
        setArrivalTime(entryData.arrivalTime)
        setDepartureTime(entryData.departureTime)
        setArrivalChecked(!!entryData.arrivalTime)
        setDepartureChecked(!!entryData.departureTime)
      } else {
        setError('Entrada no encontrada')
      }
    } catch (error) {
      setError('Error al cargar entrada')
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setError(data.error || 'Error al actualizar entrada')
      }
    } catch (error) {
    ...
    return (
      <AppLayout>
        <div className="text-center py-12">
          ...
          <a href="/entries" className="text-indigo-600 hover:text-indigo-900 mt-4 inline-block">
            ← Volver a Entradas
          </a>
        </div>
      </AppLayout>
  ...
  return (
    <AppLayout>
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-900">Editar Entrada/Salida</h1>
          <p className="mt-2 text-sm text-gray-700">
            Modifica los datos de la entrada o registra la salida.
          </p>
        </div>
